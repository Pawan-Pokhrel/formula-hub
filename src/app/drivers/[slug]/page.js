import { getDriverImagePath, getTeamLogoPath } from '@/components/schedule/scheduleHelpers';
import { DRIVER_CATALOG, getDriverBySlug } from '@/lib/data/driversCatalog';
import Image from 'next/image';
import Link from 'next/link';
import { notFound } from 'next/navigation';
import { FaArrowLeft, FaBullseye, FaCalendarAlt, FaFlag, FaTrophy } from 'react-icons/fa';
...
export default function DriverDetailPage({ params }) {
	...
	const teammate = DRIVER_CATALOG.find(
		(candidate) => candidate.teamName === driver.teamName && candidate.slug !== driver.slug
	);
			...
			<div className="relative z-10 max-w-[1300px] mx-auto pb-12 animate-fade-in">
				...
				>
					<div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
							...
							<div className="mt-3 flex items-center justify-between gap-3">
								...
								{teamLogo && (
									<div className="relative w-12 h-12 rounded-lg border border-white/12 bg-black/40 overflow-hidden">
										<Image src={teamLogo} alt={driver.teamName} fill className="object-contain p-2" />
									</div>
								)}
							</div>
						</div>
						...
						<div>
							...
							<h1 className="mt-1 text-3xl md:text-4xl font-black tracking-tight">{driver.fullName}</h1>
							<p className="mt-2 text-sm text-gray-300 max-w-3xl">{driver.bio}</p>

							<div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2.5">
								<div className="rounded-xl border border-white/10 bg-black/40 p-3">
									<p className="text-[10px] uppercase text-gray-400 tracking-wide">Championships</p>
								...
								<div className="rounded-xl border border-white/10 bg-black/40 p-3">
									<p className="text-[10px] uppercase text-gray-400 tracking-wide">Career Points</p>
									<p className="mt-1 text-xl font-bold">{driver.careerPoints.toLocaleString()}</p>
								</div>
								<div className="rounded-xl border border-white/10 bg-black/40 p-3">
									<p className="text-[10px] uppercase text-gray-400 tracking-wide">Career Starts</p>
									<p className="mt-1 text-xl font-bold">{driver.careerStarts}</p>
								</div>
								<div className="rounded-xl border border-white/10 bg-black/40 p-3">
									<p className="text-[10px] uppercase text-gray-400 tracking-wide">Experience</p>
							...
							<div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2.5">
								{[
									...
									<div key={item.label} className="rounded-xl border border-white/10 bg-black/40 p-3">
										<p className="text-[10px] uppercase text-gray-400 tracking-wide">{item.label}</p>
				...
				<section className="mt-4 grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-4">
					<div className="rounded-2xl border border-white/12 bg-black/50 p-5">
						...
						<div className="mt-5">
							<h3 className="text-sm uppercase tracking-[0.14em] text-gray-400">Career Timeline</h3>
							<div className="mt-3 space-y-2">
								{timeline.map((item) => (
									...
									>
										...
										<p className="text-sm font-semibold text-white text-right">{item.value}</p>
									</div>
								))}
							</div>
						</div>
					...
					<div className="rounded-2xl border border-white/12 bg-black/50 p-5">
						...
						<div className="mt-3 space-y-2 text-sm">
							<p className="rounded-xl border border-white/10 bg-black/35 px-3 py-2.5 text-gray-200">
								<span className="text-gray-400">Born:</span> {formatBirthDate(driver.birthDate)}
							</p>
							<p className="rounded-xl border border-white/10 bg-black/35 px-3 py-2.5 text-gray-200">
								<span className="text-gray-400">Birthplace:</span> {driver.placeOfBirth}
							</p>
							<p className="rounded-xl border border-white/10 bg-black/35 px-3 py-2.5 text-gray-200">
								<span className="text-gray-400">Nationality:</span> {driver.nationality}
							</p>
							<p className="rounded-xl border border-white/10 bg-black/35 px-3 py-2.5 text-gray-200">
								<span className="text-gray-400">Current Team:</span> {driver.teamName}
							</p>
							{teammate && (
								<p className="rounded-xl border border-white/10 bg-black/35 px-3 py-2.5 text-gray-200 inline-flex items-center gap-2">
									...
									<span className="text-gray-400">Teammate:</span> {teammate.fullName}
								</p>
							)}
						</div>
					</div>
